	...
	m_pump(eApp, 1)
{
	...
	m_last_seek_pos = 0;
	m_media_lenght = 0;
	m_useragent = "Enigma2 HbbTV/1.1.1 (+PVR+RTSP+DL;teamblue;;;)";
	m_extra_headers = "";
...
RESULT eServiceMP3::unpause()
{
	...
	if ((int)m_currentTrickRatio == 1 && !m_paused)
	{
...
RESULT eServiceMP3::trickSeek(gdouble ratio)
{
	...
	bool unpause = ((int)m_currentTrickRatio == 1 && (int)ratio == 1);
	if (unpause)
	{
...
std::string eServiceMP3::getInfoString(int w)
{
	...
	case sTagDate:
		...
		{
			gchar res[5];
			snprintf(res, sizeof(res)+1, "%04d", g_date_get_year(date));
			g_date_free(date);
		...
		else if (gst_tag_list_get_date_time(m_stream_tags, GST_TAG_DATE_TIME, &date_time))
		{
			...
			{
				gchar res[5];
				snprintf(res, sizeof(res)+1, "%04d", gst_date_time_get_year(date_time));
