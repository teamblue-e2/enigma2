...
int eRTSPStreamClient::getOrbitalPosition(int fe, int src)
{
	...
	for (int i = 0; i < 144; i++)
	{
		...
		eDebug("%d: orbital position %d, m_slot_mask %d, lnNum %d, old OP %d", i, op1, lnb_param.m_slot_mask, (int)lnb_param.m_satellites.size(), lnb_param.SatCR_idx); //lnb_param.LNBNum, lnb_param.old_orbital_position);
		if (current_src >= src)
		{
...
{
	eDebug("%s: operation %d, pid_str %s (len pids: %d)", __FUNCTION__, op, pid_str.c_str(), (int)pids.size());
		...
		if (it3 != active_services.end())
		{
			...
			{ // cache the pid, next time we won't be needed to create the pmt handler
				...
				if (apid != -1) // keep code consistent with streamdvb.cpp: eDVBServicePlay::selectAudioStream
				{
					...
					service->setCacheEntry(eDVBService::cAC4PID, apidtype == eDVBAudio::aAC4 ? apid : -1);
					if (vpid == -1)
						continue;
				}
...
{
	eDebug("update pids called: %d pids in the queue", (int)pids.size());
