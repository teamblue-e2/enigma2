...
class SleepTimerEdit(ConfigListScreen, Screen):
	...
	def createSetup(self):
		...
		if int(config.usage.inactivity_timer.value):
			self.list.append((sub1 + _("Specify timeframe to ignore inactivity sleeptimer"),
			...
			if config.usage.inactivity_timer_blocktime.value:
				self.list.append((sub2 + _("Set blocktimes by weekday"),
				...
				if config.usage.inactivity_timer_blocktime_by_weekdays.value:
					for i in range(7):
						self.list.append((sub3 + [_("Monday"), _("Tuesday"), _("Wednesday"), _("Thursday"), _("Friday"), _("Saturday"), _("Sunday")][i],
							config.usage.inactivity_timer_blocktime_day[i]))
						if config.usage.inactivity_timer_blocktime_day[i].value:
							self.list.append((sub6 + _("Start time to ignore inactivity sleeptimer"),
							...
							self.list.append((sub6 + _("End time to ignore inactivity sleeptimer"),
							...
							self.list.append((sub6 + sub1 + _("Specify extra timeframe to ignore inactivity sleeptimer"),
							...
							if config.usage.inactivity_timer_blocktime_extra_day[i].value:
								self.list.append((sub6 + sub2 + _("Extra start time to ignore inactivity sleeptimer"),
								...
								self.list.append((sub6 + sub2 + _("Extra end time to ignore inactivity sleeptimer"),
					...
					self.list.append((sub3 + _("Start time to ignore inactivity sleeptimer"),
						config.usage.inactivity_timer_blocktime_begin,
						_("Specify the start time when the inactivity sleeptimer should be ignored")))
					self.list.append((sub3 + _("End time to ignore inactivity sleeptimer"),
						config.usage.inactivity_timer_blocktime_end,
						_("Specify the end time until the inactivity sleeptimer should be ignored")))
					self.list.append((sub4 + _("Specify extra timeframe to ignore inactivity sleeptimer"),
						config.usage.inactivity_timer_blocktime_extra,
						_("When enabled you can specify an extra timeframe when the inactivity sleeptimer is ignored. Not the detection is disabled during this timeframe but the inactivity timeout is disabled")))
					if config.usage.inactivity_timer_blocktime_extra.value:
						self.list.append((sub5 + _("Extra start time to ignore inactivity sleeptimer"),
							config.usage.inactivity_timer_blocktime_extra_begin,
							_("Specify the extra start time when the inactivity sleeptimer should be ignored")))
						self.list.append((sub5 + _("Extra end time to ignore inactivity sleeptimer"),
		...
		if int(config.usage.standby_to_shutdown_timer.value):
			self.list.append((sub1 + _("Specify timeframe to ignore the shutdown in standby"),
			...
			if config.usage.standby_to_shutdown_timer_blocktime.value:
				self.list.append((sub2 + _("Start time to ignore shutdown in standby"),
				...
				self.list.append((sub2 + _("End time to ignore shutdown in standby"),
		...
		self.list.append((_("Wakeup timer"),
